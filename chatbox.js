const url_backend_base = "http://localhost:10718/";

function uuidv4() {
  var rslt = "10000000-1000-4000-8000-100000000000".replace(/[018]/g, (c) =>
    (
      c ^
      (crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (c / 4)))
    ).toString(16)
  );
  return rslt.replaceAll("-", "");
}

const cUserUniqueId = uuidv4();

const chatboxminimized = {
  backgroundColor: "transparent",
  borderRadius: "10px",
  position: "fixed",
  bottom: "10px",
  right: "8px",
  width: "64px",
  height: "64px",
  opacity: 0,
};

const chatboxopened = {
  backgroundColor: "transparent",
  position: "fixed",
  bottom: "10px",
  right: "8px",
  width: "25%",
  height: "95%",
  opacity: 0,
};

const getRequestModel = (propsInfo, payloadData = {}) => {
  return {
    channel: propsInfo.channel,
    userUniqueId: cUserUniqueId,
    requestId: uuidv4(),
    payload: payloadData,
  };
};

const callBackendPost = (url_to_call, callback_func, body = "") => {
  var full_url = url_backend_base + url_to_call;
  var req = new XMLHttpRequest();

  req.open("POST", full_url);
  req.setRequestHeader("Content-type", "application/json");
  req.setRequestHeader("X-Requested-With", "XMLHttpRequest");
  req.setRequestHeader("Access-Control-Allow-Origin", "*");

  req.onreadystatechange = function (aEvt) {
    if (req.readyState == 4) {
      if (req.status == 200) {
        console.log(">>> Response Body =>", req.responseText);
        let jsn = JSON.parse(req.responseText);
        callback_func(jsn);
      } else alert("Error loading page\n");
    }
  };
  console.log(">>> Request Body =>", body);
  if (body) req.send(JSON.stringify(body));
  else req.send(body);
};

////////////////////////////////////////////////

class ChatRoot extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpened: false,
      configs: {
        styleMinimized: chatboxminimized,
        styleMaximized: chatboxopened,
      },
      screenmsgs:[]
    };
  }

  componentDidMount() {
    callBackendPost(
      "chatconfig",
      (data) => this.setConfigFromRemote(data),
      getRequestModel(this.props)
    );
  }
  ////////////////////////////////////////////////

  setConfigFromRemote(configData) {
    this.setState((state) => ({
      configs: configData,
    }));
  }

  toggleChatWindow = () => {
    this.setState((state) => ({
      isOpened: !state.isOpened,
    }));
  };

  appendToScreenMsgs(from_whom, msg_display){
    var newelement = {
      whosemsg: from_whom,
      data: msg_display
    }

    this.setState(prevState => ({
      screenmsgs: [...prevState.screenmsgs, newelement]
    }))

  }

  sendMsgToBackend = (message) =>  {
    this.appendToScreenMsgs("client", message);
    console.log("sendMsgToBackend teitklendi", message);
    callBackendPost(
      "chatmessage",
      (data) => this.setServerMessage(data),
      getRequestModel(this.props, { message: message } )
    );

  };
  setServerMessage(serverMsg){
    console.log("setServerMessage teitklendi", serverMsg);
    this.appendToScreenMsgs("server", serverMsg);
  }

  render() {
    if (this.state.isOpened) {
      return (
        <>
          <ChatWindow
            Configs={this.state.configs}
            ClickMinimize={this.toggleChatWindow}
            SendMsgFunc={this.sendMsgToBackend}
            DisplayMsgs={this.state.screenmsgs}
          />
        </>
      );
    }
    return (
      <>
        <img
          src={this.state.configs.iconMinimized}
          alt="Want to chat"
          style={this.state.configs.styleMinimized}
          onClick={this.toggleChatWindow}
        />
      </>
    );
  }
}

////////////////////////////////////////////////
const styleMainContainer = {
  // display: "flex",
  // "flex-direction": "column",
  width: "100%",
  height: "100%",
};

const styleChildTopPart = {
  //backgroundColor: "red",
  //margin: "auto",
  width: "100%",
  height: "30px",
};

const styleChildMiddlePart = {
  //backgroundColor: "green",
  //margin: "auto",
  width: "100%",
  height: "100%",
};

const styleChildBottomPart = {
  //backgroundColor: "grey",
  //margin: "auto",
  bottom: "10px",
  width: "100%",
  height: "100px",
};

const styleDivCommon = {
  width: "100%",
  height: "100%",
};

class ChatWindow extends React.Component {
  constructor(props) {
    super(props);
  }

  render() {
    return (
      <div style={this.props.Configs.styleMaximized}>
        <table style={styleMainContainer}>
          <tr style={styleChildTopPart}>
            <td>
              <ChatWindowTopPart
                Configs={this.props.Configs}
                ClickMinimize={this.props.ClickMinimize}
              />
            </td>
          </tr>
          <tr style={styleChildMiddlePart}>
            <td>
              <ChatWindowMiddlePart DisplayMsgs={this.props.DisplayMsgs} />
            </td>
          </tr>
          <tr style={styleChildBottomPart}>
            <td>
              <ChatWindowBottomPart SendMsgFunc={this.props.SendMsgFunc} />
            </td>
          </tr>
        </table>
      </div>
    );
  }
}

/////////////////////////////////////////////////////////
const styleTopImg = {
  width: "24px",
  height: "24px",
  position: "absolute",
  top: "6px",
  right: "10px",
  cursor: "pointer",
};
class ChatWindowTopPart extends React.Component {
  constructor(props) {
    super(props);
  }
  /////

  render() {
    return (
      <div>
        <span>{this.props.Configs.textHeader}</span>
        <img
          src="data:image/svg+xml;base64,"
          alt="Minimize"
          style={styleTopImg}
          onClick={this.props.ClickMinimize}
        />
      </div>
    );
  }
}

/////////////////////////////////////////////////////////
const styleBottomPartText = {
  width: "85%",
  height: "100%",
};

const styleBottomPartImg = {
  width: "32px",
  height: "100%",
};

class ChatWindowBottomPart extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      textEntered: "",
    };
  }
  /////
  handleTextChange = (event) => {
    var tmpVal = event.target.value;
    this.setState((state) => ({
      textEntered: tmpVal,
    }));
  }

  handleKeyDown = (event) => {
    if(event.keyCode == 13 && event.shiftKey == false) {
      event.preventDefault();
      this.setState((state) => ({
        textEntered: "",
      }));
      this.props.SendMsgFunc(this.state.textEntered);
    }
  }

  render() {
    return (
      <div>
        <textarea type="text" rows="4" name="body"
          style={styleBottomPartText}
          value={this.state.textEntered}
          onChange={this.handleTextChange}
          onKeyDown={this.handleKeyDown}
        />
        <img
          alt="Enter"
          style={styleBottomPartImg}
          src="data:image/svg+xml;base64,"
        />
      </div>
    );
  }
}

/////////////////////////////////////////////////////////

const styleMiddlePartMsgAtLeft = {
  width: "100%",
  display: "flex",
  flexDirection: "column",
  alignContent: "flex-start",
  backgroundColor:"white",
  borderRadius: "7px",
  paddingTop:"2px",
}
const styleMiddlePartMsgAtRight = {
  width: "100%",
  display: "flex",
  flexDirection: "column",
  alignContent: "flex-start",
  backgroundColor:"green",
  borderRadius: "7px",
  paddingTop:"2px",
}

class ChatWindowMiddlePart extends React.Component {
  render() {
    return (
      <>
        <div>
          {this.props.DisplayMsgs.map(d => {
            if(d.whosemsg == 'client') return <div style={styleMiddlePartMsgAtRight}>{d.data}</div>
            else if(d.whosemsg == 'server') return <div style={styleMiddlePartMsgAtLeft}>{d.data.Message}</div>
          })} 
        </div>
      </>);
  }
}

/////////////////////////////////////////////////////////
// class MyComponent extends React.Component {
//   render() {
//     return <div>This is a simple component</div>;
//   }
// }
